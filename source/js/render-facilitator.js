...
function renderController(gs) {
  ...
  if (v === "r2_result") {
    ...
    return shell({ round: "Round 2", phase: "Result", say: `${ROUND2.attentionManagementWhy} ${friendLine} ${groupLine} ${videoLine}`, ask: "What did Jordan choose to know now, what was deliberately left for later, and what can no longer interrupt the day?", doNow: "Read the three consequences in order: friend information, group chat, videos. Make the distinction between LATER and MUTE explicit before moving on.", studentsDo: "Notice that the sort changed what information Jordan has now and what can still return later.", changes: `Friend known before lunch: ${gs.friendKnownBeforeLunch ? "yes" : "no"}. Group chat: ${gs.groupChatStatus}. Videos: ${gs.videosStatus}. Energy: ${ROUND2.attentionManagementGain > 0 ? "+" : ""}${ROUND2.attentionManagementGain} for the deliberate plan itself.`, next: { label: "Start Round 3 →", action: "startR3" } });
  }
